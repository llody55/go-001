...
(id, device_no, device_name, model_spec, full_scale, accuracy_class, use_dept, keeper, status, cycle_months, create_time, update_time)
SELECT 1, 'JL-Y-001', '普通压力表', 'Y-100', 10.0, 1.6, '动力车间', '王保国', 0, 12,
       '2026-09-12 09:00:00', '2026-09-12 09:00:00'
WHERE NOT EXISTS (SELECT 1 FROM t_cali_device WHERE id = 1);
INSERT INTO t_cali_device
(id, device_no, device_name, model_spec, full_scale, accuracy_class, use_dept, keeper, status, cycle_months, create_time, update_time)
SELECT 2, 'JL-Y-002', '精密压力表', 'YB-150', 25.0, 0.4, '质检中心', '李检', 0, 6,
       '2026-09-12 09:00:00', '2026-09-12 09:00:00'
WHERE NOT EXISTS (SELECT 1 FROM t_cali_device WHERE id = 2);
INSERT INTO t_cali_device
(id, device_no, device_name, model_spec, full_scale, accuracy_class, use_dept, keeper, status, cycle_months, create_time, update_time)
SELECT 3, 'JL-Y-003', '压力变送器', '3051', 1.6, 0.25, '合成车间', '赵仪', 0, 12,
       '2026-09-12 09:00:00', '2026-09-12 09:00:00'
WHERE NOT EXISTS (SELECT 1 FROM t_cali_device WHERE id = 3);
INSERT INTO t_cali_device
(id, device_no, device_name, model_spec, full_scale, accuracy_class, use_dept, keeper, status, cycle_months, create_time, update_time)
SELECT 4, 'JL-Y-004', '旧型压力表', 'Y-60', 1.6, 2.5, '检修班', '孙修', 1, 12,
       '2026-09-12 09:00:00', '2026-09-12 09:00:00'
